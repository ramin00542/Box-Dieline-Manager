...
## ADR-017 — Public Access Architecture (Locked)
**Status:** Accepted
**Decision:** All public (unauthenticated) access to template and file data
MUST go through a Next.js API Route using Supabase Service Role — never
directly from the client to Supabase. The flow is:

1. Public user requests `/api/share/[token]` with their share token.
2. The API Route validates the token against `templates.public_share_token`
   using Supabase Service Role (server-side only — never exposed to client).
3. The API Route verifies `share_expires_at > NOW()` and `deleted_at IS NULL`.
4. After successful validation, the API Route returns only non-sensitive fields
   (name, dimensions, box_type, material, files metadata) — `public_share_token`
   is NEVER included in the response.
5. For file downloads (`/api/share/[token]/files/[fileId]/download`), the API
   Route additionally verifies that the requested `fileId` belongs to the
   template matching the token, then generates a short-lived Signed URL (5 min
   TTL) using Supabase Admin client and returns it.
6. The existing public RLS policies on `templates` and `files` are REMOVED —
   all public access is via the API Route with Service Role. Authenticated
   (admin) access continues to use RLS normally.

**Reason:** RLS alone cannot securely validate a client-supplied token for
public access — it requires comparing the token sent by the caller against
`public_share_token` in the row, which is not possible with standard RLS
`USING` clauses. A server-side API Route provides:
- Full control over token validation logic
- Column-level filtering (token never returned)
- Secure Signed URL generation (Service Role stays server-side)
- File-to-template ownership verification
- Rate limiting enforcement

**Affected components:**
- `GET /api/share/[token]` — returns template + file metadata (no token)
- `GET /api/share/[token]/files/[fileId]/download` — returns Signed URL
- Remove RLS policies: "Public can view via share token", "Public can view
  files via template share token"
- Authenticated (admin) RLS policies remain unchanged
